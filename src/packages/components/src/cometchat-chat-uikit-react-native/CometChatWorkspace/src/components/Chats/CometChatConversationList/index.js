...
import CometChatUserListWithMessages from "../../Users/CometChatUserListWithMessages";
...
class CometChatConversationList extends React.Component {
  ...
  componentDidMount() {
    this.props.startChatListLoading();
      ...
      if (
        ...
        if (groupKey > -1) {
          ...
          if (conversationList.length === 0) {
            ...
            // console.log("No chats found in ChatList Component");
          }
        }
      }
    } catch (error) {
  ...
  getConversations = () => {
    new CometChatManager()
      ...
      .then((user) => {
        ...
        this.ConversationListManager.fetchNextConversation()
          .then((conversationList) => {
            ...
            //   this.decoratorMessage = 'No chats found';
            //   console.log("No chats found in ChatList Component");
            //   this.props.stopChatListLoading();
            // }
            this.setState({
              conversationList: [
          ...
          .catch((error) => {
            ...
            console.log("Error in getting chats in ChatList Component");
            const errorCode = error?.message || 'ERROR';
            this.dropDownAlertRef?.showMessage('error', errorCode);
            logger(
      ...
      .catch((error) => {
        // this.decoratorMessage = 'Error';
        console.log("Error in getting logged in user in ChatList Component")
        logger(
  ...
  itemSeparatorComponent = ({ leadingItem }) => {
    ...
    // );
    return (
      <View style={styles.itemSeparatorComponentStyle}></View>
    )
  };
  ...
  render() {
    // userList is added here so it should bend in with chatList
    const listFooterComponent = (updatedSuggestedUsersChatList) => (
      <CometChatUserListWithMessages
        suggestedUsers={this.props.suggestedUsers}
        // startChatListLoading={this.props.startChatListLoading}
        // stopChatListLoading={this.props.stopChatListLoading}
        startUserListLoading={this.props.startUserListLoading}
        stopUserListLoading={this.props.stopUserListLoading}
        navigation={this.props.navigation}
        updatedSuggestedUsersChatList={updatedSuggestedUsersChatList}
      />
    )
    const chatList = this.state.conversationList;
    const suggestedUsers = this.props.suggestedUsers;
    let suggestedUsersChatList = []; // This list is for excluding conversation that are from cometchat only and not from app
    const SuggestedUserIds = suggestedUsers.map((user) => String(user.attributes.friend_id));
    suggestedUsersChatList = chatList.filter((chat) => SuggestedUserIds.includes(chat.conversationWith.uid));
    let updatedSuggestedUsersChatList = [];
    suggestedUsersChatList.forEach((chat) => {
      suggestedUsers.forEach((user) => {
        if (chat.conversationWith.uid == user.attributes.friend_id) {
          updatedSuggestedUsersChatList.push({
            ...chat,
    ...
    // console.log("SuggestedUserIds", JSON.stringify(SuggestedUserIds))
    // console.log("suggestedUsersChatList", JSON.stringify(suggestedUsersChatList))
        ...
        <SafeAreaView style={{ backgroundColor: 'white' }}>
          <KeyboardAvoidingView
            ...
            <SwipeListView
              ...
              // data={suggestedUsersChatList}
              data={updatedSuggestedUsersChatList}
              ...
              renderItem={({ item, index }) => {
                if(index == updatedSuggestedUsersChatList.length - 1)
                return (
                  <>
              ...
              ListFooterComponent={listFooterComponent(updatedSuggestedUsersChatList)}
