...
class CometChatMessages extends React.PureComponent {
  ...
  onChangeScore = (data) => {
    console.log('Game score changed 1', data.userGameScore, data.friendGameScore)
    this.setState({ userGameScore: data.userGameScore, friendGameScore: data.friendGameScore });
  };
  
  onSetUserName = (data) => {
    this.setState({ userName: data.userName, frdName: data.frdName });
  };
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const params = navigation?.state?.params || this.props;
    const prevParams = prevNavigation?.state?.params || prevProps;
  ...
  deleteGroup = (group) => {
    this.setState(
      ...
      () => {
        this.props.navigation?.state?.params?.actionGenerated('groupDeleted', group) ||
  ...
  liveReactionReceived = (reaction) => {
    try {
      ...
      if (reaction.data.type === enums['METADATA_TYPE_LIVEREACTION']) {
        const params = this.props?.navigation?.state?.params || this.props;
        ...
        <KeyboardAvoidingView
          ...
          <SafeAreaView style={style.chatWrapperStyle}>
            ...
            <CometChatMessageHeader
              ...
              loggedInUser={params.loggedInUser}
              actionGenerated={this.actionHandler}
              userGameScore={this.state.userGameScore}
              friendGameScore={this.state.friendGameScore}
              userName={this.state.userName}
              frdName={this.state.frdName}
