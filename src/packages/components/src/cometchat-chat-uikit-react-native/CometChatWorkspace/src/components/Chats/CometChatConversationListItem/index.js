    ...
    try {
      ...
      switch (lastMessage.type) {
        case CometChat.MESSAGE_TYPE.TEXT:
          let string = lastMessage.text;
          let trueString = '--true';
          let falseString = '--false';
          let matchTrue = string.includes(trueString);
          let matchFalse = string.includes(falseString);
          if (matchTrue || matchFalse) {
            let array = string.split("--");
            if (lastMessage.receiver.uid == this.props.loggedInUser.uid) {
              message = `I think they would choose a ${array[0]}`;
            } else {
              message = `I would choose to ${array[0]}`;
            }
          } else {
  ...
  render() {
    ...
    if (this.state.lastMessage) {
      lastMessageTimeStamp = (
        <Text 
        style={
          this.props.conversation.unreadMessageCount > 0
            ? {
                ...styles.itemLastMsgTimeStyle,
                color: this.props.theme.color.pink,
              }
            : styles.itemLastMsgTimeStyle
        }
         numberOfLines={1}>
    ...
    //   );
    // }
    // console.log("CCCC", JSON.stringify(this.props.conversation))
    return (
        ...
        <TouchableOpacity
          ...
          <View style={styles.itemThumbnailStyle}>
            <CometChatAvatar
              ...
              image={{ uri: this.props.conversation.photo }}
              cornerRadius={25}
