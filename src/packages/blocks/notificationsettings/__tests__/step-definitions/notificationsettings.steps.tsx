...
defineFeature(feature, (test) => {
    ...
    test('User navigates to notificationsettings', ({ given, when, then }) => {
        ...
        then('RestAPI will return an error', () => {
            ...
            instance.notificationAPICallId = msgRegistrationErrorRestAPI
            runEngine.sendMessage("Unit Test", msgRegistrationErrorRestAPI)
        });
        then('I can select the checkbox button with with out errors', () => {
    
            let wrapper = shallow(<Notificationsettings navigation={navigation} id={""} />)

            let customInput = wrapper.findWhere((node) => node.prop("testID") === "CustomCheckBox");
            let event = { target: { value: "bcbhjvhj" } };
            customInput.props().onChangeValue(event);
    ...
    test('Set notification settings sub group active or inactive', ({ given, when, then }) => {
        ...
        then('notification settings will return without error', () => {
            let msgRegistrationErrorRestAPI = new Message(getName(MessageEnum.RestAPIResponceMessage))
            msgRegistrationErrorRestAPI.addData(getName(MessageEnum.RestAPIResponceDataMessage), msgRegistrationErrorRestAPI);
            msgRegistrationErrorRestAPI.addData(getName(MessageEnum.RestAPIResponceSuccessMessage),
            {
            ...
            instance.setNotificationSubGroupAPICall = msgRegistrationErrorRestAPI
            runEngine.sendMessage("Unit Test", msgRegistrationErrorRestAPI)
        });
    });
