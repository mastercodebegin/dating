...
defineFeature(feature, (test) => {
    ...
    beforeEach(() => {
        jest.resetModules()
        jest.doMock('react-native', () => ({ Platform: { OS: 'macOS' } }))
        jest.spyOn(helpers, 'getOS').mockImplementation(() => 'macOS');
    });
    ...
    test('User navigates to Notifications', ({ given, when, then }) => {
        ...
        then('Notifications will load with out errors', () => {
            ...
            let testScrollView = notificationsBlock.findWhere((node) => node.prop('testID') === "testScrollView");
            let refreshView = testScrollView.dive();
            let testingRefreshControl = refreshView.findWhere((node) => node.prop('testID') === "testingRefreshControl");
            testingRefreshControl.props().onRefresh();

            let plans = notificationsBlock.findWhere((node) => node.prop('testID') === "suggestedFriendShowModalWindow");
            plans.simulate('press')
            
            let plans2 = notificationsBlock.findWhere((node) => node.prop('testID') === "userListID");
            ...
            plans2.props().keyExtractor(item)
            plans2.props().ItemSeparatorComponent()
            const renderItemComponent = plans2.renderProp('renderItem')({item})
            let touchableNotification = renderItemComponent.findWhere((node) => node.prop('testID') === "testTouchableNotification");
            touchableNotification.simulate('press');
            ...
            instance.getDataCallId = getNotificationAPI.messageId
