...
class CountryCodeSelectorTable extends BlockComponent<Props, S, SS> {
  ...
  searchFilterFunction = (text: string) => {
    ...
    const newData = this.arrayholder.filter(item => {
      const itemData = `${item.attributes.name.toUpperCase()} (${item.id}) +${item.attributes.country_code
        }`;
  ...
  actionOnRow(item: any) {
    ...
    setTimeout(function () {
      ...
      let countryNameCode = ` ${item.attributes.emoji_flag} ${item.attributes.name
    ...
    return (
      <View style={{ flex: 1 }}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableWithoutFeedback testID="actionOnRow" onPress={() => this.actionOnRow(item)}>
              <ListItem
                title={` ${item.attributes.emoji_flag} ${item.attributes.name
                  } (${item.id})`}
